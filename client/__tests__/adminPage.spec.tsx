...
jest.mock("@/utils/addPog", () => {
  ...
  return {
    ...originalModule,
    addPog: jest.fn((name, price, tickerSymbol, color) =>
...
describe("Admin", () => {
  ...
  it("should add a new pog", async () => {
    ...
    (addPog as jest.MockedFunction<typeof addPog>).mockImplementation(
      mockAddPog
    );
    ...
    const tickerSymbolInput = screen.getByPlaceholderText(
      "Enter Pog Ticker Symbol"
    );
    ...
    await waitFor(() =>
      expect(addPog).toHaveBeenCalledWith("Rabbit", 100, "PGRAAH", "Yellow")
    );
    expect(
      screen.queryByPlaceholderText("Enter Pog Name")
  ...
  it("should delete a pog", async () => {
    ...
    (
      fetchAllPogs as jest.MockedFunction<typeof fetchAllPogs>
    ).mockResolvedValue(pogs);
    render(<Admin />);
